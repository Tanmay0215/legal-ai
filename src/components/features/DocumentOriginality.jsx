...
export function DocumentOriginality({ document }) {
  ...
  const analyzeDocument = async () => {
    if (!document) {
      setAnalysis({
        error: true,
        message: "Please upload a document first to analyze originality.",
      });
      return;
    }
    ...
    try {
      ...
      setAnalysis({
        error: true,
        message:
          "Document originality analysis is not yet implemented in the backend API. This feature would require additional endpoints for plagiarism detection and similarity analysis.",
      });
    } catch (error) {
      setAnalysis({
        error: true,
        message: "Failed to analyze document originality: " + error.message,
      });
    } finally {
          ...
          <div className="space-y-6">
            {analysis.error ? (
              <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                ...
                <p className="text-sm text-yellow-700">{analysis.message}</p>
                <div className="mt-4">
                  <Button
                    variant="outline"
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1 text-muted-foreground">
                      <p>
                        Total Words:{" "}
                        {analysis.metadata?.totalWords?.toLocaleString()}
                      </p>
                      <p>
                        Unique Phrases:{" "}
